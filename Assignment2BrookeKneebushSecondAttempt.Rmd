...
ModelA<-randomForest(family_income~ work_retail, data = Thanksgiving_Meals_LMfct,importance=TRUE, na.action=na.omit)
...
ModelB<-randomForest(family_income~ work_retail + community_type +main_dish, importance=TRUE, data = Thanksgiving_Meals_fct,na.action=na.omit)
...
ModelC<-randomForest(family_income~ work_retail + main_dish + cranberry + pie3, importance=TRUE, data = Thanksgiving_Meals_fct,na.action=na.omit)
...
## ModelC / ModelCC is the best because the OOB estimate of  error rate is the lowest: 79.09% and tidy(ModelCC) provides estimates closer to 1.
